...
/*
 * Return a string representation of a TCP segment header.
...
char *tcpHdrToString(tcpheader *hdr) {
    ...
    snprintf(buf, MAXLENGTH, "%s%s%s%sCkSum: 0x%04x Seq: %d (%08x) Ack: %d (%08x) Win: %d",
	     getSyn(hdr) ? "SYN " : "", 
	     getAck(hdr) ? "ACK " : "", 
	     getFin(hdr) ? "FIN " : "", 
	     getRst(hdr) ? "RST " : "", hdr->checksum, ntohl(hdr->seqNo), ntohl(hdr->seqNo), ntohl(hdr->ackNo), ntohl(hdr->ackNo), ntohs(hdr->windowSize));
    return buf;
}
