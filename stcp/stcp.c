 ...
 * 'r'eceived packet.  The packet is assumed to be in network byte order.
 */
...
// If you are computing the checksum of a message contained in a packet
// structure pointed to by a variable pkt, then you could call this function
// using: 
//
//   ipchecksum(pkt->data, pkt->len);
//
// or equivalently:
//
//   ipchecksum(pkt->hdr, pkt->len);
//
// since pkt->hdr and pkt->data should be the same address.
 ...
 * the fields of the header.  The checksum is set to 0.  If data is not NULL
 * then len bytes of data are copied into the payload portion of the STCP
 * segment.
 *
 * To create a segment that will have no data (a SYN or ACK-only segment,
 * for example), use:
 *     createSegment(&pkt, flags, rwnd, seq, ack, 0, 0);
 *
 * To create a segment that contains some data (from the input file, for
 * example), use:
 *     createSegment(&pkt, flags, rwnd, seq, ack, datapointer, len);
 */
void createSegment(packet *pkt, int flags, unsigned short rwnd, unsigned int seq, unsigned int ack, unsigned char *data, int len) {
    initPacket(pkt, 0, len + sizeof(tcpheader));
